...
public class BCCThem implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        int row= temp.getTable().getSelectedRow();
        System.out.println(temp.getTable().getValueAt(row, 1).toString());
        if(temp.getTable().getValueAt(row, 2).toString().equals("Đã chấm công")) {
        	JOptionPane.showMessageDialog(temp,"Nhân viên "+ temp.getTable().getValueAt(row, 1).toString()+temp.getThang().getSelectedItem()+"/"+temp.getNam().getSelectedItem()+" đã được chấm công!! Vui lòng chọn nhân viên khác");
        }
        else {
           ...
           app.getData().getDanhSachBangChamCong().getDataFromDatabase();
            ...
            int row_num= temp.getTable().getRowCount();
            for(int i=0; i<row_num; i++) {
            	if(temp.getTable().getValueAt(i, 2).equals("Chưa chấm công")) {
            		temp.getTable().setRowSelectionInterval(i, i);
            		temp.getMa_Ten().setText(temp.getTable().getValueAt(i, 1).toString());
            		break;
            	}
            }
        }
